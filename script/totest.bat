...
if %ERRORLEVEL% neq 0 (
    ...
    echo [DEBUG] Staying on test branch for manual handling
    echo You can now manually resolve the issue or try again
    echo Current branch: test
    exit /b 1
)
...
echo [CMD] git merge !CURRENT_BRANCH! --no-ff --no-edit
...
if %ERRORLEVEL% neq 0 (
    ...
    echo [DEBUG] Staying on test branch for manual handling
    echo You can now manually resolve the push issue or try again
    echo Current branch: test
    exit /b 1
)
